...
(->
	...
	isIE = ->
		...
		(if (myNav.indexOf("msie") isnt -1) then parseInt(myNav.split("msie")[1]) else false)
	window.mobilecheck = ->
		check = false
		((a, b) ->
	...
	unless window.mobilecheck()
		if not isIE() | isIE() > 8
			elements = document.getElementsByClassName("js-view") # apply plugin to images with js-view class name
			Intense elements
)()
